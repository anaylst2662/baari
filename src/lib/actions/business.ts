...
function done(salonId: number) {
  revalidatePath(`/business/${salonId}`, "layout");
}
  ...
  if (booking && (to === "confirmed" || to === "rejected")) {
    ...
    await sendMessage(
      ...
      to === "confirmed"
        ...
        : `Baari: sorry, ${salon.name} can't take your booking on ${when}. Pick another time: ${appUrl(`/book/${salon.slug}`)}`,
    );
  }
  done(salon.id);
}
...
/** My Salon → Salon details. */
export async function updateDetails(_prev: ProfileState, formData: FormData): Promise<ProfileState> {
  const { salon } = await access(formData);
  try {
    await db.update(schema.salons).set(parseDetails(formData)).where(eq(schema.salons.id, salon.id));
  } catch {
    return { error: "Please fill in the name, address and area." };
  }
  done(salon.id);
  return { saved: true };
}

/** My Salon → Opening hours. */
export async function updateHours(_prev: ProfileState, formData: FormData): Promise<ProfileState> {
  const { salon } = await access(formData);
  await db.update(schema.salons).set({ hours: parseHours(formData) }).where(eq(schema.salons.id, salon.id));
  done(salon.id);
  return { saved: true };
}

/** My Salon → Photos (links, one per line). */
export async function updatePhotos(_prev: ProfileState, formData: FormData): Promise<ProfileState> {
  const { salon } = await access(formData);
  await db.update(schema.salons).set({ photos: parsePhotos(formData) }).where(eq(schema.salons.id, salon.id));
  done(salon.id);
  return { saved: true };
}

export async function registerSalon(_prev: ProfileState, formData: FormData): Promise<ProfileState> {
  const user = await requireUser("/business/join");
  let data;
  try {
    data = { ...parseDetails(formData), hours: parseHours(formData), photos: parsePhotos(formData) };
  } catch {
    return { error: "Please fill in the salon name, address and area." };
  }
  ...
  for (const phone of adminPhones()) {
    await sendMessage(phone, `Baari admin: new salon "${salon.name}" (${salon.area}) awaits approval. ${appUrl("/admin/salons")}`);
  }
  redirect(`/business/${salon.id}?welcome=1`);
}
