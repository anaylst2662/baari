...
import { getReviews, getSalonBySlug, getServices, getStaff } from "@/lib/data";
...
import { PhoneIcon, PinIcon, UsersIcon } from "@/components/icons";
...
export default async function SalonPage({ params }: PageProps<"/s/[slug]">) {
  ...
  const [services, staff, reviews, queue] = await Promise.all([
        ...
        <div>
          ...
          {staff.length > 0 && (
            <p className="mt-3 text-sm text-slate-600">
              <span className="font-semibold">{t.staff}:</span> {staff.map((s) => s.name).join(", ")}
            </p>
          )}
        </div>
        {salon.lat !== null && salon.lng !== null && (
