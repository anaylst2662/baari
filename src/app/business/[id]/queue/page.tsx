...
import { addWalkIn, callNext, setQueueStatus } from "@/lib/actions/business";
import { AutoRefresh, SubmitButton } from "@/components/client";
import { Empty, WaitPill } from "@/components/ui";

export default async function BusinessQueue({ params }: PageProps<"/business/[id]/queue">) {
  const { id } = await params;
  const { salon } = await requireSalonAccess(Number(id));
  const { t, lang } = await getDict();
  if (salon.mode === "booking") return <Empty>{t.queueOffNote}</Empty>;
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="space-y-3">
          ...
          <form id="add-walk-in" action={addWalkIn} className="card flex scroll-mt-24 flex-wrap items-end gap-2 p-3">
            {hidden}
